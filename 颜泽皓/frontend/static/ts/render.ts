export type WordEntryTy = { [key: string]: [number, number] | string };
...
function render_word_block(
    sentence_div: HTMLDivElement,
    sentence: string,
    start: number,
    end: number,
    type?: string,
...
export function render_sentence(
    data: { [key: string]: WordEntryTy[] | string },
    sentence: string
) {
    ...
    let entries = <WordEntryTy[]>data["entities"];
    let most_unrelated = <string>data["MUE"];
    ...
    for (let index = 0; index < entries.length; index++) {
        ...
        const next_entry_start =
            (index < entries.length - 1) ? entries[index + 1]["range"][0] : sentence.length;

        render_word_block(
            sentence_div,
            sentence,
            <number>entry["range"][0],
            <number>entry["range"][1],
            <string>entry["type"],
            index
        );
        for (let j = <number>entry["range"][1]; j < next_entry_start; j++) {
            render_word_block(sentence_div, sentence, j, j + 1);
        }
    }
...
export function render_related_words(data: [string, number][]) {
    let inHtml: string = "<h1>相关词</h1>";
...
export function render_relations(data: string[], chosen_index: number) {
    let inHtml: string = "<h1>关系</h1>";
    let sentence = sessionStorage.getItem("sentence");
    let entities_range: [number, number][] =
        JSON.parse(sessionStorage.getItem("entities"));
    let chosen_entity = sentence.substring(
        entities_range[chosen_index][0],
        entities_range[chosen_index][1]
    );
    ...
    for (let index = 0; index < data.length; index++) {
        ...
        let entity = sentence.substring(
            entities_range[index][0],
            entities_range[index][1]
        );
